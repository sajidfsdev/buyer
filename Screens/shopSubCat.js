...
const ShopSubCat=(props)=>{
    ...
    const moveToNext=(subCat)=>{
        ...
        products_RP.forEach(elem=>{
            if(elem.cat===cat && elem.subCat===subCat)
            {
                if(elem.subSubCat==="not available")
                {
                    noSubCat=true;
                    //return Alert.alert("No subSubcat","No subSubCat");
                ...
                else
                {
                    ...
                    {
                        subSubCatArr.push(elem.subSubCat);
                    }
                    else
                    {
                        //checking for possible duplication
                        let duplicate=false;
                        subSubCatArr.forEach(c=>{
                            if(elem.subSubCat===c)
                            {
                                duplicate=true;
                            }
        ...
        if(noSubCat===false)
        {
        console.log("******************************subSubSubCat Calculated Array Are");
        console.log(subSubCatArr);

        props.navigation.navigate("shopSubSubCat",{
            vendorId:products_RP[0].vendorId,
            outletName:props.navigation.getParam("outletName"),
            cat:props.navigation.getParam("cat"),
            subCat:subCat,
            subSubCat:[...subSubCatArr],
            headerText:props.navigation.getParam("cat")+"-->"+subCat
        });
           ...
           <View style={styles.container}>
               ...
               {/* Top Navigation Ends Here...... */}
                    ...
                    <FlatList
                        ...
                        data={props.navigation.getParam("subCat")}
                        numColumns={3}
...
ShopSubCat.navigationOptions=(navigationData)=>{
